...
@kyc_user_api.route("/test-upload-complete", methods=["POST"])
def test_upload_complete():
    """Test upload KYC completo senza autenticazione per debug."""
    try:
        # Simula dati di test
        user_id = 2
        doc_type = "driver_license"
        rel_front = "test_front.png"
        rel_back = "test_back.png"
        
        # Usa la stessa logica dell'upload reale
        from backend.models.kyc import KYCRequest, KYCStatus
        from backend.shared.database import get_connection
        
        # 1) Registra la richiesta KYC
        req = KYCRequest(
            user_id=user_id,
            doc_type=doc_type,
            file_front=rel_front,
        ...
        with get_connection() as conn, conn.cursor() as cur:
            ...
            def insert_doc(title: str, rel_path: str):
                if not rel_path:
                    return
                cur.execute("""
                    INSERT INTO documents (user_id, category_id, title, file_path, visibility, verified_by_admin)
                    VALUES (%s, %s, %s, %s, 'admin', FALSE)
                """, (user_id, cat['id'] if cat else None, title, rel_path))
            
            # Inserisci documenti
            insert_doc(f"Documento {cat['name'] if cat else doc_type} - fronte", rel_front)
            insert_doc(f"Documento {cat['name'] if cat else doc_type} - retro", rel_back)
            
            # 3) Imposta lo stato utente a pending
            cur.execute("UPDATE users SET kyc_status='pending' WHERE id=%s AND kyc_status IN ('unverified','rejected')",
                       (user_id,))
        ...
        return jsonify({
            "success": True,
            "message": "Test upload KYC completo funziona",
            "request_id": req.id,
            "doc_type": doc_type
        })
